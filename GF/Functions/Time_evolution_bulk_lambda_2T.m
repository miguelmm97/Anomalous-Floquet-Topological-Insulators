function  [prod]=Time_evolution_bulk_lambda_2T(delta,kx,ky,n_t,dt,J_1,J_2,J_3, lambda)
...
H1=@(delta,kx,ky)   [delta/2, (lambda*J_1)*exp(1i*ky)+J_2*exp(-1i*0.5*(sqrt(3)*kx+ky))+J_3*exp(-1i*0.5*(-sqrt(3)*kx+ky));  % Hamiltonian first step
                    (lambda*J_1)*exp(-1i*ky)+J_2*exp(1i*0.5*(sqrt(3)*kx+ky))+J_3*exp(1i*0.5*(-sqrt(3)*kx+ky)), -delta/2];    
