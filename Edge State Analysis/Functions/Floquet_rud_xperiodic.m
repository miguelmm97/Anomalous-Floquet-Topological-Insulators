function [eigenstates, eigenvalues]=Floquet_rud_xperiodic(kx, N_layers, n_t, dt, J_1, J_2, J_3, delta, termination)
...
switch termination
    ...
    case 'Zigzag'
        ...
        % Hamiltonians for each driving step
        H1=[0.5*delta*diag(diagonal), gamma1 ;
            conj(transpose(gamma1)), -0.5*delta*diag(diagonal)];
        H1=[0.5*delta*diag(diagonal),  gamma2 ;
            conj(transpose(gamma2)), -0.5*delta*diag(diagonal)];
        H1=[0.5*delta*diag(diagonal),gamma3 ;
            conj(transpose(gamma3)), -0.5*delta*diag(diagonal)];
