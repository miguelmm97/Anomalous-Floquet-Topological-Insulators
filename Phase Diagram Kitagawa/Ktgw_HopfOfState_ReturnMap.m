...
% applying it to honeycomb, 3stage Kitagawa drive from the PRB, i.e. increase tunneling to lambda*J   
...
% modified from StepWiseHxgn_HopfOfState_ReturnMap.m   

function Ktgw_HopfOfState_ReturnMap
global J1 J2 J3 T deltaA lambda
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% -------------------------- INPUT --------------------------------
J=1;     % everything is in terms of J
w=4.5;  % driving frequency omega, hbar*omega/J, T=2pi/w 
delta=2;   % detuning. for initial sublattice bias, divided equally btwn A and B sublattices so delta_B=delta/2, delta_A=-delta_B

% N=1;    % your period will be N*T

lambda=3;   %tunneling along the i^th direction will be lambda*J during the i^th period
...
H1=@(kx,ky) ([deltaA, (-J1*lambda -J2*exp(1i/2*(kx+sqrt(3)*ky)) -J3*exp(1i/2*(-kx+sqrt(3)*ky)) ) ; (-J1*lambda -J2*exp(-1i/2*(kx+sqrt(3)*ky)) -J3*exp(-1i/2*(-kx+sqrt(3)*ky)) ), deltaB]);         
H2=@(kx,ky) ([deltaA, (-J1 -J2*lambda*exp(1i/2*(kx+sqrt(3)*ky)) -J3*exp(1i/2*(-kx+sqrt(3)*ky)) ) ; (-J1 -J2*lambda*exp(-1i/2*(kx+sqrt(3)*ky)) -J3*exp(-1i/2*(-kx+sqrt(3)*ky)) ), deltaB]);
H3=@(kx,ky) ([deltaA, (-J1 -J2*exp(1i/2*(kx+sqrt(3)*ky)) -J3*lambda*exp(1i/2*(-kx+sqrt(3)*ky)) ) ; (-J1 -J2*exp(-1i/2*(kx+sqrt(3)*ky)) -J3*lambda*exp(-1i/2*(-kx+sqrt(3)*ky)) ), deltaB]);


h1mag=@(kx,ky) sqrt(deltaA^2 +(-J1*lambda-J2*cos((kx+sqrt(3)*ky)/2)-J3*cos((-kx+sqrt(3)*ky)/2))^2 +(J2*sin((kx+sqrt(3)*ky)/2)+J3*sin((-kx+sqrt(3)*ky)/2))^2 ); 
h2mag=@(kx,ky) sqrt(deltaA^2 +(-J1-J2*lambda*cos((kx+sqrt(3)*ky)/2)-J3*cos((-kx+sqrt(3)*ky)/2))^2 +(J2*lambda*sin((kx+sqrt(3)*ky)/2)+J3*sin((-kx+sqrt(3)*ky)/2))^2 ); 
h3mag=@(kx,ky) sqrt(deltaA^2 +(-J1-J2*cos((kx+sqrt(3)*ky)/2)-J3*lambda*cos((-kx+sqrt(3)*ky)/2))^2 +(J2*sin((kx+sqrt(3)*ky)/2)+J3*lambda*sin((-kx+sqrt(3)*ky)/2))^2 );


U1=@(kx,ky,tt) ( cos(h1mag(kx,ky)*tt)*eye(2) -1i*sin(h1mag(kx,ky)*tt) *H1(kx,ky)/h1mag(kx,ky) );  
U2=@(kx,ky,tt) ( cos(h2mag(kx,ky)*tt)*eye(2) -1i*sin(h2mag(kx,ky)*tt) *H2(kx,ky)/h2mag(kx,ky) );   % BE CAREFUL when calling, THESE t ARE THE EVOLUTION TIMES, delta_t YANI, time spent during this stage only
U3=@(kx,ky,tt) ( cos(h3mag(kx,ky)*tt)*eye(2) -1i*sin(h3mag(kx,ky)*tt) *H3(kx,ky)/h3mag(kx,ky) );



z_i=[1; 0];    %initial state, in sublattice basis [1; 0]

z_fun=@(kx,ky,t) U1(kx,ky,t)*z_i;   % wave function during the first stage. I need to call this function;  %during the 1stage, I'll calculate the w.f.s by using this function

tic
%% ------------------- 1st period 0-->T ---------------------------------
% filename=[pwd '/data/Hopf_w' num2str(w) '_d' num2str(delta) '_lambda' num2str(lambda) '_dk' num2str(dk) '_tN' num2str(tN) '_derk' num2str(derk) '_dert' num2str(dert) '_J1' num2str(J1) '_J2' num2str(J2) '_J3' num2str(J3) '.mat'];
    ...
    for cnt=0:3*tN  
        switch cnt
            case tN  %switch to the 2nd stage
                z_fun=@(kx,ky,t) U2(kx,ky,t-T/3)* U1(kx,ky,T/3) *z_i;     %function during the 2nd stage. t=how much you evolve only during the 2nd stage
%                 cnt, %pause
            case 2*tN  %3rd stage
                z_fun=@(kx,ky,t) U3(kx,ky,t-2*T/3)* U2(kx,ky,T/3)* U1(kx,ky,T/3) *z_i;     
%                 cnt, %pause
...
for cky=1:kNy    % to calculate integrand for dky integral  
    ...
    for ckx=1:kNx    % to calculate integrand of dkx integral 
        ...
        zT=U3(kx,ky,T/3)* U2(kx,ky,T/3)* U1(kx,ky,T/3) *z_i;    %states at the end of normal evolution time T
        zx_minus=U3(kx-derk/2,ky,T/3)* U2(kx-derk/2,ky,T/3)* U1(kx,ky,T/3) *z_i; 
        zx_plus=U3(kx+derk/2,ky,T/3)* U2(kx+derk/2,ky,T/3)* U1(kx,ky,T/3) *z_i; 
        zy_minus=U3(kx,ky-derk/2,T/3)* U2(kx,ky-derk/2,T/3)* U1(kx,ky,T/3) *z_i;
        zy_plus=U3(kx,ky+derk/2,T/3)* U2(kx,ky+derk/2,T/3)* U1(kx,ky,T/3) *z_i;
...
H1=[deltaA, (-J1*lambda -J2*exp(1i/2*(kx+sqrt(3)*ky)) -J3*exp(1i/2*(-kx+sqrt(3)*ky)) ) ; (-J1*lambda -J2*exp(-1i/2*(kx+sqrt(3)*ky)) -J3*exp(-1i/2*(-kx+sqrt(3)*ky)) ), -deltaA];         
H2=[deltaA, (-J1 -J2*lambda*exp(1i/2*(kx+sqrt(3)*ky)) -J3*exp(1i/2*(-kx+sqrt(3)*ky)) ) ; (-J1 -J2*lambda*exp(-1i/2*(kx+sqrt(3)*ky)) -J3*exp(-1i/2*(-kx+sqrt(3)*ky)) ), -deltaA];
H3=[deltaA, (-J1 -J2*exp(1i/2*(kx+sqrt(3)*ky)) -J3*lambda*exp(1i/2*(-kx+sqrt(3)*ky)) ) ; (-J1 -J2*exp(-1i/2*(kx+sqrt(3)*ky)) -J3*lambda*exp(-1i/2*(-kx+sqrt(3)*ky)) ), -deltaA];
