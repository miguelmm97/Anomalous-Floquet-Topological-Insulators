...
omega=[7:-step:3]; %  Frequency (units of J)
periods=2*pi./omega; % Period 
delta=[0:0.1:2]; % Potential offset vector
lambda=[3]; % Degree of anisotropy (lambda=1 identified with high frequency static regime)
...
for z=1:length(delta)
    ...
    for j=1:length(omega)
        ...
        for i=1:length(kx_gamma_K);
            ...
            U=Time_evolution_bulk_lambda(delta(z),kx_gamma_K(i),ky_gamma_K(i),n_t, dt,J_1,J_2,J_3, lambda); % Floquet Hamiltonian (H*T)
            HF=1i*logm(U);
        ...
        for i=1:length(kx_KK);
            ...
            U=Time_evolution_bulk_lambda(delta(z), kx_KK(i), ky_KK(i), n_t, dt,J_1,J_2,J_3, lambda); % Floquet Hamiltonian (H*T)
            HF=1i*logm(U);
            HF_diag=eig(HF); % Diagonal Floquet Hamiltonian
        ...
        for i=1:length(kx_K_gamma);
            ...
            U=Time_evolution_bulk_lambda(delta(z), kx_K_gamma(i), ky_K_gamma(i), n_t, dt,J_1,J_2,J_3, lambda); % Floquet Hamiltonian (H*T)
            HF=1i*logm(U);
            HF_diag=eig(HF); % Diagonal Floquet Hamiltonian
    ...
    if cont_0>1
        q0=top_charge_0(cont_0, singularities_0, step, n_t, delta(z),J_1,J_2,J_3); % Assign a charge to each singularity
        
    else
        q0=0;
    end   
    if cont_pi>1
        qpi=top_charge_pi(cont_pi, singularities_pi, step, n_t, delta(z),J_1,J_2,J_3); % Assign a charge to each singularity
        ...
        if phase_diag(z,j,3)==0 && phase_diag(z,j,4)==0 ;
            plot( omega(j), delta(z), '.b','MarkerSize',5)
                ...
                plot(omega(j), delta(z), '.g','MarkerSize',5)
                    ...
                    plot(omega(j), delta(z), '.','Color',[0 0.8 0.2],'MarkerSize',5)
...
title(['Phase Diagram for \lambda=', num2str(lambda)])
