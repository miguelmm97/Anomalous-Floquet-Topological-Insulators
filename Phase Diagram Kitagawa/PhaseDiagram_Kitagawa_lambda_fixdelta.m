...
omega=[7:-0.02:4.5]; %  Frequency (units of J) % Period 
...
for z=1:length(omega)
    ...
    for j=1:length(lambda)
        ...
        for i=1:length(kx_gamma_K);
            ...
            U=Time_evolution_bulk_lambda(delta,kx_gamma_K(i),ky_gamma_K(i),n_t, dt,J_1,J_2,J_3, lambda(j)); % Floquet Hamiltonian (H*T)
            HF=1i*logm(U);
        ...
        for i=1:length(kx_KK);
            ...
            U=Time_evolution_bulk_lambda(delta, kx_KK(i), ky_KK(i), n_t, dt,J_1,J_2,J_3, lambda(j)); % Floquet Hamiltonian (H*T)
            HF=1i*logm(U);
            HF_diag=eig(HF); % Diagonal Floquet Hamiltonian
        ...
        for i=1:length(kx_K_gamma);
            ...
            U=Time_evolution_bulk_lambda(delta, kx_K_gamma(i), ky_K_gamma(i), n_t, dt,J_1,J_2,J_3, lambda(j)); % Floquet Hamiltonian (H*T)
            HF=1i*logm(U);
            HF_diag=eig(HF); % Diagonal Floquet Hamiltonian
    ...
    if cont_pi>1
        qpi=top_charge_pi_lambda(cont_pi, singularities_pi, step, n_t, delta,J_1,J_2,J_3); % Assign a charge to each singularity
    ...
    for j=1:length(lambda)
        ...
        phase_diag(z,j,:)=[omega(z), lambda(j), W_0, W_pi];
            ...
            plot( lambda(j), omega(z), '.b','MarkerSize',8)
                ...
                plot(lambda(j), omega(z), '.g','MarkerSize',8)
                    ...
                    plot(lambda(j), omega(z), '.c','MarkerSize',8)
                        ...
                        plot(lambda(j), omega(z), '.r','MarkerSize',8)
                            ...
                            plot(lambda(j), omega(z), '.m','MarkerSize',8)
                                ...
                                plot(lambda(j), omega(z), '.y','MarkerSize',8)
                                    ...
                                    plot(lambda(j), omega(z), '.w','MarkerSize',8)
                                end
                            end
                        end
                    end
...
plot([3.82:0.01:4.99],5.9*ones(1, length([3.82:0.01:4.99])),'.c','MarkerSize',8)
plot([3.8:0.01:4.99],5.86*ones(1, length([3.8:0.01:4.99])),'.c','MarkerSize',8)
plot([4:0.01:4.99],4.54*ones(1, length([4:0.01:4.99])),'.c','MarkerSize',8)
plot([4:0.01:4.99],4.58*ones(1, length([4:0.01:4.99])),'.c','MarkerSize',8)
plot([4:0.01:4.99],4.62*ones(1, length([4:0.01:4.99])),'.c','MarkerSize',8)
plot([4:0.01:4.99],4.66*ones(1, length([4:0.01:4.99])),'.c','MarkerSize',8)
plot([4:0.01:4.99],4.70*ones(1, length([4:0.01:4.99])),'.c','MarkerSize',8)
...
print -djpeg -r600 phasedigram2.png
