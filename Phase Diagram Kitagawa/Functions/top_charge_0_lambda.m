function [q0]=top_charge_0_lambda(cont_0, singularities_0, step, n_t, delta,J_1,J_2,J_3)


% Getting rid of redundant gap closings

control_0=compare_lambda(singularities_0(:,1),step);
...
for j=1:length(control_0(:,1))
    ...
    lambda=singularities_0(indice,1);
    kx=singularities_0(indice,2);
    ky=singularities_0(indice,3);   
    dt=singularities_0(indice,4);   
    h_0=0;
    q0(cont,:)=[ lambda, S_matrix_lambda(lambda,kx,ky,delta,n_t,dt,h_0,J_1,J_2,J_3)];
    cont=cont+1;
end
end